class p2DebugDraw {
    ...
    private COLOR_D_SLEEP: number = 0xffffff;//0x999999;
    private COLOR_D_WAKE: number = 0xffffff;//0xe5b2b2;
    private COLOR_K: number = 0xffffff;//0x7f7fe5;
    private COLOR_S: number = 0xffffff;//0x7fe57f;
    private COLOR_BLACK: number = 0xffffff;//0x000000;
    private COLOR_RED: number = 0xffffff;
    ...
    public constructor(world: p2.World,sprite:egret.Sprite,height:number) {
        this.world = world;
        this.sprite = sprite;
        this.STAGEHEIGHT = height;
    }
    public drawDebug(except:number[]): void {
        //alert(except);
        ...
        for (var i: number = 0; i < l; i++) {
           ...
           //alert(body.id);
            var isValid = true; 
            for(var k: number = 0; k < except.length; k++)  if(body.id === except[k]) isValid = false;
    ...
    private drawConvexShape(shape: p2.Convex, b: p2.Body, color: number,fillColor:boolean): void {
        ...
        this.drawConvex(worldVertices, color, 0.9, fillColor);
    }
    ...
    public drawSegment(start: number[], end: number[], color: number): void {
        ...
        this.sprite.graphics.moveTo(start[0]*this.factor, this.STAGEHEIGHT - start[1]*this.factor);
        this.sprite.graphics.lineTo(end[0]*this.factor, this.STAGEHEIGHT - end[1]*this.factor);
    }
    ...
    public drawCircle(pos: number[], radius: number, color: number, alpha: number= 1,fillColor?:boolean): void {
        ...
        this.sprite.graphics.drawCircle(pos[0]*this.factor, this.STAGEHEIGHT - pos[1]*this.factor, radius*this.factor);
        this.sprite.graphics.endFill();
    }

    public drawLineConvex(vertices: number[][], color: number, alpha: number= 1,fillColor:boolean=true): void {
        this.sprite.graphics.lineStyle(1, color);
     ...
     public drawConvex(vertices: number[][], color: number, alpha: number= 1,fillColor:boolean=true): void {
        ...
        var l: number = vertices.length;
        var worldPoint: number[] = vertices[0];
        this.sprite.graphics.moveTo(worldPoint[0]*this.factor, this.STAGEHEIGHT - worldPoint[1]*this.factor);
        for (var i: number = 1; i <= l; i++) {
            worldPoint = vertices[i % l];
            this.sprite.graphics.lineTo(worldPoint[0]*this.factor, this.STAGEHEIGHT - worldPoint[1]*this.factor);
        }
        this.sprite.graphics.endFill();
    }
